  ...
  return (
    <>
      <header className="flex justify-between items-center border-b-2 border-slate-300 dark:border-slate-600 py-3 mb-6 px-2 sm:px-0">
        <h1 className="text-xl font-bold">Вгадай Колір</h1>
        <div className="flex gap-3">
          ...
          <IconButton title="Переключити тему" onClick={toggleTheme}>
            {theme === "dark" ? <IcRoundDarkMode /> : <IcRoundLightMode />}
          </IconButton>

          <IconButton title="Правила гри" onClick={toggleIsRulesModal}>
            <IcRoundHelpOutline />
          </IconButton>
        </div>
      </header>
